# -*- coding: utf-8 -*-
"""
Created on Wed Feb 19 19:39:09 2020
...
def output():
    ...
    #df["time_3"] = [datetime.datetime.strptime(x, '%Y-%m-%d%H:%M:%S.%f') for x in df.time_2]
    #df["time"] = df["time_3"]
    ...
    #with open(r"share/alerts/{}-UC1A-Attach.json".format(datetime.datetime.now().strftime("%Y-%m-%d.%H.%M.%S.%f")),"w") as f:
    ...
    for i in df_M.index:  
        ...
        if x>U_C:
            ...
            Dic =  {"Use Case": "UC1A-Attach",'detect-time': str(B) , 'Aggragated Count' : str(A)}
    ...
    #with open(r"share/alerts/2020-02-12-063012-UC1A-Attach-XXX.json", "w") as outfile:
